...
## Створення виконуваного файлу (.exe) для Windows
Для генерації виконуваного файлу запустіть скрипт `setup.bat`. 
Скрипт перевірить наявність віртуального середовища, встановить необхідні бібліотеки та створить `AcademicMatch.exe`. Після завершення файл знаходитиметься у папці `dist`.